...
const COL_TEN_CANDIDATES = ["Tên Bệnh nhân", "Tên bệnh nhân"];
const COL_NGUON_CANDIDATES = ["Nguồn"];
const COL_TIEN_CANDIDATES = ["Tiền sau miễn giảm"];
...
async function readXlsx(buffer: ArrayBuffer): Promise<ParsedTable> {
...
function readCsv(text: string): ParsedTable {
...
function buildByPatient(
  ...
  const total = Math.round(
    Array.from(sums.values()).reduce((a, b) => a + b, 0)
  );
  ...
  for (const name of names) {
    ws.addRow({ ten: name, tien: Math.round(sums.get(name) || 0) });
  }
...
function buildBySource(
  ...
  sources.forEach((source, idx) => {
    const inner = grouped.get(source)!;
    const sourceTotal = Math.round(
      Array.from(inner.values()).reduce((a, b) => a + b, 0)
    );
...
export async function POST(request: NextRequest) {
  try {
    ...
    const mode = String(formData.get("mode") || "patient");
    ...
    if (!isCsv && !isXlsx) {
      return NextResponse.json(
        { error: "Chỉ hỗ trợ file .xlsx hoặc .csv" },
        { status: 400 }
      );
    }
    ...
    if (!colTen) {
      return NextResponse.json(
        {
          error: `Không tìm thấy cột 'Tên Bệnh nhân'. Các cột hiện có: ${table.columns.join(
            ", "
          )}`,
        },
        { status: 400 }
    ...
    if (!colTien) {
      return NextResponse.json(
        {
          error: `Không tìm thấy cột 'Tiền sau miễn giảm'. Các cột hiện có: ${table.columns.join(
            ", "
          )}`,
        },
        { status: 400 }
    ...
    if (mode === "source") {
      ...
      if (!colNguon) {
        return NextResponse.json(
          {
            error: `Không tìm thấy cột 'Nguồn'. Các cột hiện có: ${table.columns.join(
              ", "
            )}`,
          },
          { status: 400 }
